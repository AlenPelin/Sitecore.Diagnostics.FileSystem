        ...
        private class TestDataForWriteAllLines
        {
            ...
            public static IEnumerable ForDifferentEncoding
            {
                get
                {
                    ...
                    Action writeArray = () => fileSystem.Internals.File.WriteAllLines(Path, fileContentArray);
                    Action writeArrayUtf32 = () => fileSystem.Internals.File.WriteAllLines(Path, fileContentArray, Encoding.UTF32);
            ...
            private static IEnumerable GetCasesForArgumentChecking(string path)
            {
                ...
                var fileContentArray = fileContentEnumerable.ToArray();
                TestDelegate writeEnumberable = () => fileSystem.Internals.File.WriteAllLines(path, fileContentEnumerable);
                TestDelegate writeEnumberableUtf32 = () => fileSystem.Internals.File.WriteAllLines(path, fileContentEnumerable, Encoding.UTF32);
                TestDelegate writeArray = () => fileSystem.Internals.File.WriteAllLines(path, fileContentArray);
                TestDelegate writeArrayUtf32 = () => fileSystem.Internals.File.WriteAllLines(path, fileContentArray, Encoding.UTF32);
            ...
            private static IEnumerable ForNullEncoding
            {
                get
                {
                    ...
                    var fileContentEnumerable = new List<string>();
                    var fileContentArray = fileContentEnumerable.ToArray();
                    TestDelegate writeEnumberableNull = () => fileSystem.Internals.File.WriteAllLines(Path, fileContentEnumerable, null);
                    TestDelegate writeArrayNull = () => fileSystem.Internals.File.WriteAllLines(Path, fileContentArray, null);
            ...
            public static IEnumerable ForPathIsDirectory
            {
                get
                {
                    ...
                    fileSystem.Internals.Directory.CreateDirectory(path);
                    var fileContentEnumerable = new List<string>();
                    var fileContentArray = fileContentEnumerable.ToArray();
                    TestDelegate writeEnumberable = () => fileSystem.Internals.File.WriteAllLines(path, fileContentEnumerable);
                    TestDelegate writeEnumberableUtf32 = () => fileSystem.Internals.File.WriteAllLines(path, fileContentEnumerable, Encoding.UTF32);
