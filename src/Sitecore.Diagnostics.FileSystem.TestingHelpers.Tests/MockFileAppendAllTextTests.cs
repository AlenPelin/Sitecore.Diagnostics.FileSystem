namespace Sitecore.Diagnostics.FileSystem.TestingHelpers.Tests
{
    ...
    public class MockFileAppendAllTextTests
    {
        ...
        public void MockFile_AppendAllText_ShouldCreateIfNotExist()
        {
            ...
            // Act
            fileSystem.Internals.File.AppendAllText(path, " some text");

            // Assert
            Assert.AreEqual(
                "Demo text content some text",
                fileSystem.Internals.File.ReadAllText(path));
        }
        ...
        public void MockFile_AppendAllText_ShouldCreateIfNotExistWithBom()
        {
            ...
            // Act
            fileSystem.Internals.File.AppendAllText(path, "AA", Encoding.UTF32);
        ...
        public void MockFile_AppendAllText_ShouldFailIfNotExistButDirectoryAlsoNotExist()
        {
            ...
            // Assert
            Exception ex;
            ex = Assert.Throws<DirectoryNotFoundException>(() => fileSystem.Internals.File.AppendAllText(path, "some text"));
            Assert.That(ex.Message,
                Is.EqualTo(String.Format(CultureInfo.InvariantCulture, "Could not find a part of the path '{0}'.", path)));

            ex =
                Assert.Throws<DirectoryNotFoundException>(
                    () => fileSystem.Internals.File.AppendAllText(path, "some text", Encoding.Unicode));
            Assert.That(ex.Message,
                Is.EqualTo(String.Format(CultureInfo.InvariantCulture, "Could not find a part of the path '{0}'.", path)));
        }
